...
var Z = require('..');
require('sanctuary-type-classes').lte = Z.lte;
...
function ListArb(arb) {
  return jsc.array(arb).smap(function(xs) {
    return Z.reduce(function(xs, x) { return Cons(x, xs); }, Nil, xs);
  });
}
...
function listToArray(xs) {
  ...
  return Z.reduce(function(xs, x) { xs.push(x); return xs; }, [], xs);
}
    ...
    testLaws({
      ...
      Extend: {
        ...
        laws: {
          associativity: [
            jsc.array(jsc.number),
            jsc.constant(function(xs) { return Z.reduce(function(x, y) { return x * y; }, 1, xs); }),
            jsc.constant(sum)
          ]
        }
      }
